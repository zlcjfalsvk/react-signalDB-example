...
export function TodoForm({ 
  ...
  const [formData, setFormData] = useState<TodoFormData>({
    ...
    dueDate: initialData?.dueDate ? 
      new Date(initialData.dueDate.getTime() - initialData.dueDate.getTimezoneOffset() * 60000)
  ...
  const validateField = useCallback((name: keyof TodoFormData, value: string): string | undefined => {
    switch (name) {
      case 'title':
      ...
      case 'priority':
        if (!PRIORITY_OPTIONS.find(p => p.value === value)) return 'Invalid priority';
        return undefined;
  ...
  const handleInputChange = useCallback((name: keyof TodoFormData, value: string) => {
  ...
  const handleBlur = useCallback((name: keyof TodoFormData) => {
