...
export function useSignalDB<T extends { id: string }>({
  ...
  // CRUD operations
  const findOne = useCallback((findSelector: object): T | null => {
    return collection.findOne(findSelector);
  }, [collection]);

  const insert = useCallback((doc: Omit<T, 'id'>): string => {
    const id = generateId();
  ...
  const updateOne = useCallback((updateSelector: object, update: Partial<T>): number => {
    const updateDoc = {
      ...update,
      updatedAt: new Date(),
    };
  ...
  const updateMany = useCallback((updateSelector: object, update: Partial<T>): number => {
    const updateDoc = {
      ...update,
      updatedAt: new Date(),
    };
  ...
  const removeOne = useCallback((removeSelector: object): number => {
    return collection.removeOne(removeSelector);
  }, [collection]);

  const removeMany = useCallback((removeSelector: object): number => {
    return collection.removeMany(removeSelector);
  }, [collection]);

  const exists = useCallback((existsSelector: object): boolean => {
