...
export function useTodos(options: UseTodosOptions = {}): UseTodosReturn {
  ...
  const createTodo = useCallback(
    (todoData: Omit<Todo, 'id' | 'createdAt' | 'updatedAt'>): string => {
      const now = new Date();
  ...
  const updateTodo = useCallback(
    (id: string, update: Partial<Todo>): boolean => {
  ...
  const getTodoStats = useCallback((): TodoStats => {
    ...
    const todayAdded = todos.filter(
      (todo) => todo.createdAt >= today && todo.createdAt < tomorrow
    ).length;
  ...
  const markAllCompleted = useCallback((): number => {
    return todosCollection.updateMany(
      { completed: false },
  ...
  const getTodosForToday = useCallback((): Todo[] => {
    ...
    return todos.filter(
      (todo) => todo.dueDate && todo.dueDate >= today && todo.dueDate < tomorrow
    );
  }, [todos]);
